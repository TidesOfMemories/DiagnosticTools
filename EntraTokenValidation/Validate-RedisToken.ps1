...
.DESCRIPTION
    ...
    Token acquisition options for testing or production diagnostics:
        ...
        • Application capture: grab the token from a debugger snapshot or secure trace collector. In
          ...
          rotate the credential afterward).
...
.NOTES
    Requires Azure CLI (az) or the Az PowerShell module to be installed and authenticated.
#>
...
function Get-DecodedJwtToken {
    ...
    try {
        ...
        if ($tokenParts.Count -ne 3) {
            throw "The provided token is not a valid JWT format. JWT tokens should have three parts separated by periods (header.payload.signature). Please verify you copied the complete token."
        }
...
function Test-TokenStructure {
    ...
    try {
        ...
        Write-ColoredMessage "Token decoded successfully" -Color Green -Prefix "[OK]"
        return $decodedToken
    }
    catch {
        $ValidationResult.AddError("Failed to decode token: $_")
        Write-ColoredMessage "Failed to decode token" -Color Red -Prefix "[ERROR]"
        return $null
    }
}
...
function Show-TokenClaims {
    ...
    if ($DecodedToken.exp) {
        ...
        if ($expirationTime -lt (Get-Date)) {
            Write-ColoredMessage "WARNING: Token has expired!" -Color Yellow -Indent 2 -Prefix "[WARN]"
        }
    }
...
function Test-TokenAudience {
    ...
    if ($audienceValid) {
        Write-ColoredMessage "Token audience '$($DecodedToken.aud)' is valid for Azure Managed Redis" -Color Green -Prefix "[OK]"
    }
    else {
        $ValidationResult.AddError("Token audience '$($DecodedToken.aud)' is not valid for Azure Managed Redis Cache")
        Write-ColoredMessage "Invalid audience" -Color Red -Prefix "[ERROR]"
...
function Test-TokenIssuer {
    ...
    if ($DecodedToken.iss -match 'https://sts\.windows\.net/([^/]+)/' -or
        ...
        $tenantId = $Matches[1]
        Write-ColoredMessage "Token issued by Microsoft Entra ID for tenant: $tenantId" -Color Green -Prefix "[OK]"
    }
    else {
        $ValidationResult.AddError("Token issuer '$($DecodedToken.iss)' does not match expected Entra ID format")
        Write-ColoredMessage "Unexpected issuer format: $($DecodedToken.iss)" -Color Yellow -Prefix "[WARN]"
    }
}
...
function Test-TokenExpiration {
    ...
    if (-not $DecodedToken.exp) {
        ...
        Write-ColoredMessage "Token does not contain expiration claim" -Color Yellow -Prefix "[WARN]"
        return
    }
    ...
    if ($expirationTime -lt (Get-Date)) {
        ...
        Write-ColoredMessage "Token expired at $expirationTime" -Color Red -Prefix "[ERROR]"
    }
    else {
        $timeRemaining = $expirationTime - (Get-Date)
        $minutesRemaining = [Math]::Floor($timeRemaining.TotalMinutes)
        Write-ColoredMessage "Token is valid until $expirationTime ($minutesRemaining minutes remaining)" -Color Green -Prefix "[OK]"
    }
}
...
function Show-ValidationSummary {
    ...
    if (-not $ValidationResult.HasErrors() -and -not $ValidationResult.HasWarnings()) {
        Write-ColoredMessage "All validations passed!" -Color Green -Prefix "[OK]"
    ...
    if (-not $ValidationResult.HasErrors()) {
        Write-ColoredMessage "Validation completed with warnings:" -Color Yellow -Prefix "[WARN]"
    ...
    # Has errors
    Write-ColoredMessage "Validation failed with errors:" -Color Red -Prefix "[ERROR]"
    foreach ($errRes in $ValidationResult.Errors) {
        Write-ColoredMessage "- $errRes" -Color Red -Indent 2
    }

    if ($ValidationResult.HasWarnings()) {
        Write-Host ""
        Write-ColoredMessage "Additional warnings:" -Color Yellow -Prefix "[WARN]"
        foreach ($warning in $ValidationResult.Warnings) {
            Write-ColoredMessage "- $warning" -Color Yellow -Indent 2
        }
    }
    ...
    if ($ValidationResult.HasErrors()) {
        ...
        Write-ColoredMessage "   - Use credential with scope: https://redis.azure.com/.default" -Color Gray -Indent 2
        ...
        # Add tip for using the correct Entra extension package for Redis client libraries
        Write-ColoredMessage "$tipNumber. Ensure your Redis client library is configured to use the correct Microsoft Entra extension package." -Color Gray -Indent 2
        Write-ColoredMessage "   - See: https://learn.microsoft.com/en-us/azure/azure-cache-for-redis/cache-azure-active-directory-for-authentication#configure-your-redis-client-to-use-microsoft-entra" -Color Gray -Indent 2
        Write-Host ""
    }
}
...
function Test-AccessPolicyAssignment {
    ...
    if (-not (Test-AzureCliInstalled)) {
        ...
        Write-ColoredMessage "Azure CLI (az) not installed. Skipping access policy validation." -Color Cyan -Prefix "[INFO]"
    ...
    if (-not $accountInfo) {
        ...
        Write-ColoredMessage "Not logged in to Azure. Run 'az login' to validate access policies" -Color Yellow -Prefix "[WARN]"
        return
    }

    Write-ColoredMessage "Logged in to Azure as: $($accountInfo.user.name)" -Color Green -Prefix "[OK]"
    ...
    if ($accountInfo.id -ne $ResourceComponents.SubscriptionId) {
        ...
        Write-ColoredMessage "CLI subscription doesn't match resource subscription. Switching..." -Color Yellow -Prefix "[WARN]"

        try {
            Set-AzureSubscription -SubscriptionId $ResourceComponents.SubscriptionId
            Write-ColoredMessage "Switched to subscription: $($ResourceComponents.SubscriptionId)" -Color Green -Prefix "[OK]"
        }
        catch {
            $ValidationResult.AddError("Could not switch to subscription: $_")
            Write-ColoredMessage "Could not switch to subscription: $_" -Color Red -Prefix "[ERROR]"
            return
        }
    }
    ...
    if (-not $tokenObjectId) {
        ...
        Write-ColoredMessage "Token missing 'oid' claim - cannot verify access policy assignment" -Color Yellow -Prefix "[WARN]"
        return
    }
    ...
    try {
        ...
        if ($result.ExitCode -ne 0) {
            ...
            if ($errorOutput -match "ResourceNotFound|NotFound|could not be found") {
                ...
                Write-ColoredMessage "Redis cache not found" -Color Red -Prefix "[ERROR]"
            ...
            elseif ($errorOutput -match "AuthorizationFailed|Forbidden|does not have authorization") {
                ...
                Write-ColoredMessage "Insufficient permissions to read access policies" -Color Yellow -Prefix "[WARN]"
                ...
                Write-ColoredMessage "Could not retrieve access policy assignments" -Color Yellow -Prefix "[WARN]"
        ...
        if ($outputString -notmatch 'objectId|user') {
            ...
            Write-ColoredMessage "No access policy assignments configured for this cache" -Color Red -Prefix "[ERROR]"
        ...
        if (Test-ObjectIdInAssignments -OutputString $outputString -ObjectId $tokenObjectId) {
            Write-ColoredMessage "Token's object ID is assigned to the cache!" -Color Green -Prefix "[OK]"
            ...
            Write-ColoredMessage "Token's object ID is NOT found in access policy assignments" -Color Red -Prefix "[ERROR]"
        ...
        Write-ColoredMessage "Could not retrieve access policy assignments: $_" -Color Yellow -Prefix "[WARN]"
    }
}
...
try {
    ...
    try {
        ...
        Write-ColoredMessage "Resource parsed successfully:" -Color Green -Prefix "[OK]"
        ...
        Write-ColoredMessage "Failed to parse resource ID: $_" -Color Red -Prefix "[ERROR]"
        exit 1
    }
    ...
    Write-Host ""
    Write-ColoredMessage "An unexpected error occurred: $_" -Color Red -Prefix "[ERROR]"
